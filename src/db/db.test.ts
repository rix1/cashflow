...
/** The tables a v2 database had, as far as the migration cares. */
const V2_SCHEMA = `
  ...
  try {
    ...
    for (const pass of [1, 2]) {
      ...
      try {
        ...
        assertEquals(
          db.prepare(
            `SELECT fingerprint, category_key, note, one_off FROM overrides`,
          )
            .all(),
          [{
            fingerprint: "fp1",
        ...
        db.exec(
          `INSERT INTO overrides(fingerprint, one_off) VALUES ('fp2', 1)`,
        );
        db.exec(`DELETE FROM overrides WHERE fingerprint = 'fp2'`);
      } finally {
        db.close();
      }
    }
