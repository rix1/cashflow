...
src/db/schema.ts            SQLite DDL (schema v2)
...
- **Totals** are defined once in `web/queries.ts` (`INCOME_EXPR`,
  `EXPENSE_EXPR`, `SAVING_EXPR`): income/expense come from category kind,
  uncategorized rows split by sign, saving and transfer kinds are excluded. Keep
  every view consistent with those expressions.
